...
public:
      ...
      : myData()
   {
       ...
       for (size_type n = 0; n < right.size(); n++)
   ...
   void resize( const size_type newSize )
   {
      ...
      if( newSize > originalSize )
      {
         if( newSize > capacity() )
         {
            ...
            myData.myFirst = new value_type[newCapacity]();
            for (size_type n = 0; n < originalSize; n++)
                myData.myFirst[n] = temp[n];
            delete[] temp;
            myData.myEnd = myData.myFirst + newCapacity;
         }
         for (size_type n = originalSize; n < newSize; n++)
             myData.myFirst[n] = 0;
...
public:
      ...
      : myData()
   {
       ...
       const_iterator it (right.myData.myHead->next);
       for (size_type n = 0; n < right.size(); n++, ++it)
           push_back(*it);
   }
   ...
   list& operator=( const list &right )
   {
       if (this != &right)
       {
           ...
           const_iterator it2(right.myData.myHead->next);
           for (size_type n = 0; n < right.size();n++, ++it1, ++it2)
               *it1 = *it2;
       }
   ...
   deque& operator=( const deque &right )
   {
      if( &right != this )
      {
          if (right.myData.mySize > 0)
          {
              ...
              if (right.size() == 32 && myData.myOff % 4 == 0);
              else
                  while (right.myData.mySize + (myData.myOff - 1) % 4 >= 4 * myData.mapSize)
                      doubleMapSize();

              typename deque< Ty >::const_iterator it(right.myData.myOff, &right.myData);
              for (int n = 0; n < right.size(); n++, ++it)
              {
                  if (myData.map[(n + myData.myOff) / 4 % myData.mapSize] == nullptr)
                      myData.map[(n + myData.myOff) / 4 % myData.mapSize] = new Ty[4]();

                  myData.map[(n + myData.myOff) / 4 % myData.mapSize][(n + myData.myOff) % 4] = *it;
              }
...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
       ...
       for (int n = 0; n < myData.mySize; n++)
           myData.map[(n + myData.myOff) / 4] = temp[(n + myData.myOff) / 4 % myData.mapSize];
       delete[] temp;
...
void HugeInteger< T >::operator*=( HugeInteger &op2 )
{
   ...
   if( isZero() || op2.isZero() )
   {
    ...
    int* temp = new int[product.size()]();

    for (size_type i = 0; i < integer.size(); i++, ++it3)
    {
        it2 = op2.integer.begin();
        
        for (size_type j = 0; j < op2.size(); j++, ++it2)          
            temp[i + j] += *it2 * *it3;
    }
    for (size_type n = 0; n < product.size(); n++, ++it1)
        *it1 = temp[n];
    
    for(it1 = product.integer.begin(); it1 != product.integer.end(); ++it1)
        if (*it1 >= 10)
        {
            it2 = it1;
            ++it2;
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger &op2 )
{
   ...
   size_type quotientSize = remainder.size() - buffer.size();
   HugeInteger ten;
   ten.integer.push_back(1);

   for (int n = 0; n < quotientSize; n++)
       buffer *= ten;
