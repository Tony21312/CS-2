...
public:
      ...
      : myData()
   {
       for (int n = 0; n < right.size(); n++)
           push_back(right.myData.map[((n + right.myData.myOff) / 4) % right.myData.mapSize][(n + right.myData.myOff) % 4]);
   }
...
void HugeInteger< T >::operator-=( HugeInteger &op2 )
{
   ...
   for (int n = 0; n < difference.size(); n++, ++it1)
       if (*it1 < 0 || *it1 > 100000)
       {
           it3 = it1;
           ++it3;
...
void HugeInteger< T >::operator*=( HugeInteger &op2 )
{
   ...
   if( isZero() || op2.isZero() )
   {
    ...
    size_type productSize = this->size() + op2.size();
    ...
    for (; it1 != this->end(); ++it1, i++)
    {
        for (j = 0, it2 = op2.begin(); it2 != op2.end(); ++it2, j++)
        {
            it3 = product.begin();
            it3 += (i + j);
            *it3 += *it1 * *it2;
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger &op2 )
{
   ...
   typename deque< T >::reverse_iterator it = quotient.rbegin();
