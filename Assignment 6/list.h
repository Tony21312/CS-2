...
public:
   ...
   list& operator=( const list &right )
   {
       if (this != &right)
       {
           ...
           nodePtr This = myData.myHead->next;
           nodePtr Right = right.myData.myHead->next;
           for (size_t n = 0; n < right.myData.mySize; n++, This = This->next, Right = Right->next)
               This->myVal = Right->myVal;
       }
      return *this;
   }
   ...
   void push_back( const Ty &val )
   {
       nodePtr temp = new node;
       temp->myVal = val;
       temp->next = myData.myHead;//temp->next point to head
       temp->prev = myData.myHead->prev;//temp->prev point to the prev of head
       myData.myHead->prev->next = temp;//the prev of head point to temp
       myData.myHead->prev = temp;//head->prev point to temp
       myData.mySize++;
   }
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    if (left.myData.mySize != right.myData.mySize)
        return false;
    ListNode< Ty >* tempL = left.myData.myHead->next;
    ListNode< Ty >* tempR = right.myData.myHead->next;
    for (; tempL != left.end(); tempL = tempL->next, tempR = tempR->next)
