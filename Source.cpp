	...
	iterator insert(const_iterator where, const value_type& val)
	{
		if (where >= myData.myFirst && where <= myData.myLast)
		{
			...
			if (originalSize == originalCapacity)
			{
				...
				for (size_t n = place; n < originalSize; n++)
...
HugeInteger< T > HugeInteger< T >::square()
{
	...
	for (size_t n = 0; n < square.integer.size(); n++)
	{
		if (square.integer[n] >= 10000)
		{
			square.integer[n + 1] += square.integer[n] / 10000;
			square.integer[n] %= 10000;
		}
		if (square.integer[n] < 0)
		{
			int temp = square.integer[n] - 2147483647;
			square.integer[n] = 2147483647;

			square.integer[n + 1] += square.integer[n] / 10000;
			square.integer[n] %= 10000;
			square.integer[n] += temp;
			if (square.integer[n] >= 10000)
			{
				square.integer[n + 1] += square.integer[n] / 10000;
				square.integer[n] %= 10000;
	...
	int position = square.integer.size();

	while (square.integer[square.integer.size() - 1] == 0)
		square.integer.erase(square.integer.begin() + position - 1);

	/*   for (int n = square.integer.size() - 1; n >= 0; n--)
		   cout << square.integer[n];
	   cout << endl;*/
	return square;
}
