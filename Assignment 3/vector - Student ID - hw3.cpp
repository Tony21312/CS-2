...
vector::vector( const vector &right )
   : myFirst(),
     ...
     myEnd()
{
    size_t rightCap = right.myEnd - right.myFirst;
    myFirst = new value_type[rightCap]();

    for (int n = 0; n < (right.myLast - right.myFirst); n++)
        myFirst[n] = right.myFirst[n];

    myEnd = myLast = myFirst + (right.myLast - right.myFirst);
}
