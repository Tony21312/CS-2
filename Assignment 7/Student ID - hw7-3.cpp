...
public:
      ...
      : myData()
   {
       ...
       for (; temp != right.myData.myHead; temp = temp->next)
...
HugeInteger< T > HugeInteger< T >::square( value_type powerTwo )
{
   ...
   for (int i = 0; i < integer.size(); i++, it2++)
   {
       it3 = integer.begin();
       for (int j = 0; j < integer.size(); j++, it3++)
           temp[i + j] += *it2 * *it3;
   }
   ...
   for (int n = 0; n < squareSize; n++, it1++)
       *it1 = temp[n];
   delete[] temp;
   it1 = square.integer.end();
   it1--;
   while (*it1 == 0)
   {
       it1--;
