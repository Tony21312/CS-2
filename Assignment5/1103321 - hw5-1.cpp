...
public:
   ...
   void resize( const size_type newSize )
   {
      ...
      if( newSize > originalSize )
      {
         ...
         for (size_t n = originalSize; n < capacity(); n++)
             myData.myFirst[n] = 0;        
      }
...
bool HugeInteger< T >::operator<( const HugeInteger< T > &right ) const
{
    ...
    for (size_t n = integer.size() - 1; n >= 0; n--)
        if (integer[n] != right.integer[n])
            return integer[n] < right.integer[n];
    return false;
} // end function operator<
...
void HugeInteger< T >::operator-=( HugeInteger< T > &op2 )
{
   ...
   for (int n = 0; n < op2.integer.size(); n++)
       difference.integer[n] -= op2.integer[n];

   for(int n = 0; n < difference.integer.size(); n++)
       if (difference.integer[n] < 0 || difference.integer[n] > 10000)
       {
           difference.integer[n] += 10;
           difference.integer[n + 1]--;
       }

   for (int n = difference.integer.size() - 1; n >= 0; n--)
   {
       if (difference.integer[n] == 0)
...
void HugeInteger< T >::operator*=( HugeInteger< T > &op2 )
{
   ...
   for(int n = 0; n < product.integer.size() - 1; n++)
       if (product.integer[n] >= 10)
       {
           product.integer[n + 1] += product.integer[n] / 10;
           product.integer[n] %= 10;
       }

   for (int n = product.integer.size() - 1; n >= 0; n--)
   {
       if (product.integer[n] == 0)
...
HugeInteger< T > HugeInteger< T >::operator/(HugeInteger< T >& op2)
{
    ...
    int n = remainder.size() - buffer.size();
    HugeInteger< T > ten(2);
    ten.integer[1] = 1;//tens digit 1
    for (int i = 0; i < n; i++)
    {        
        buffer *= ten;
        //cout << buffer << endl;
        //cout << buffer.integer.size() << "  " << buffer.integer.capacity() << endl;
    }
...
HugeInteger< T > HugeInteger< T >::operator%( HugeInteger< T > &op2 )
{
    ...
    int n = remainder.integer.size() - op2.integer.size();
    HugeInteger< T > ten(2);
    ten.integer[1] = 1;//1 is tens digit
    for (int i = 0; i < n; i++)
    {
        buffer *= ten;
        //cout << buffer << endl;
