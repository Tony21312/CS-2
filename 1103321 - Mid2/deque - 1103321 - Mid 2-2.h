...
public:
		...
		: myData()
	{
		...
		{
			myData.mapSize = 8;
			while (right.myData.mySize > (myData.mapSize - 1) * 4)
				myData.mapSize *= 2;

			myData.map = new Ty * [myData.mapSize]();
			const_iterator it = right.begin();

			for (int n = 0; n < right.myData.mySize; n++, ++it)
			{
				if (myData.map[(n + right.myData.myOff) / 4 % myData.mapSize] == nullptr)
					myData.map[(n + right.myData.myOff) / 4 % myData.mapSize] = new Ty[4];

				myData.map[(n + right.myData.myOff) / 4 % myData.mapSize][(n + right.myData.myOff) % 4] = *it;
			}
	...
	void erase(const_iterator where)
	{
		...
		if (myData.mySize > 0 && begin() <= whereIt && whereIt < end())
			...
			{
				iterator it = whereIt - 1;
				for (; whereIt != begin(); --it, --whereIt)
					*whereIt = *it;
